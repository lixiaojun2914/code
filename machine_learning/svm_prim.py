...
# 计算二次规划相关参数
Q = cvx.matrix(np.eye(D_in + 1))
p = cvx.matrix(np.array([0.0] * (D_in + 1)))
G = cvx.matrix(np.zeros((N, D_in + 1)))
h = cvx.matrix([-1.0] * N)
Q[D_in, D_in] = 0.0

for i in range(N):
    for j in range(D_in):
        G[i, j] = - y_train[i] * x_train[i][j]
    G[i, D_in] = - float(y_train[i])
sol = cvx.solvers.qp(Q, p, G, h)
